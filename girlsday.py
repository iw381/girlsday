...
preloaded_list_earth = []

for i in range(1, 28):
        LMAX_earth = int(np.exp(i/28.*5.3))

        mollviewMapReconstr = np.load("data/mollviewEarth_recon_max%d.dat"%LMAX_earth,allow_pickle=True)

        preloaded_list_earth.append(mollviewMapReconstr)


def plotAlmMap (i):
    LMAX = int(np.exp(i/28.*5.3))

    mollviewMapReconstr = preloaded_list_earth[i-1]
...
def superpose_spherical_harmonics(l0, l1m0, l1m1, l2m0, l2m1, l2m2, l3m0,l3m1,l3m2,l3m3):
    ...
    superposition = np.zeros_like(map00)
    if l0:
        superposition += 1.5*map00
    if l1m0:
        superposition += 1.5*np.loadtxt("data/triangular10.dat")
    if l1m1:
        superposition += 1.5*np.loadtxt("data/triangular11.dat")
    if l2m0:
        superposition += 1.5*np.loadtxt("data/triangular20.dat")
    if l2m1:
        superposition += 1.5*np.loadtxt("data/triangular21.dat")
    if l2m2:
        superposition += 1.5*np.loadtxt("data/triangular22.dat")
    if l3m0:
        superposition += 1.5*np.loadtxt("data/triangular30.dat")
    if l3m1:
        superposition += 1.5*np.loadtxt("data/triangular31.dat")
    if l3m2:
        superposition += 1.5*np.loadtxt("data/triangular32.dat")
    if l3m3:
        superposition += 1.5*np.loadtxt("data/triangular33.dat")
    plotEarth(superposition)
...
for i in range(1, 31):
    LMAX_cmb = int(np.exp(i/30.*6.9))

    mollviewMapReconstr = np.loadtxt("data/mollviewCMB_recon_max%d.dat"%LMAX_cmb)

    preloaded_list_cmb.append(mollviewMapReconstr)
